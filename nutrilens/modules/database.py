...
def _row_to_entry(row: sqlite3.Row) -> FoodEntry:
    ...
    return FoodEntry(
        ...
        vitamin_b1_mg=get("vitamin_b1_mg"),
        vitamin_b2_mg=get("vitamin_b2_mg"),
    ...
    try:
        cursor = conn.execute(
            ...
            INSERT INTO foods (
                ...
                added_sugars_g, vitamin_d_mcg, vitamin_b1_mg, vitamin_b2_mg,
                calcium_mg, iron_mg, potassium_mg, source, created_at
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            (
                entry.name.strip(),
