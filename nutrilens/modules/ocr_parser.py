...
def _clahe_enhanced(gray_image: Image.Image) -> Optional[Image.Image]:
    """Denoise, upscale 2x, and apply CLAHE (Contrast Limited Adaptive
    Histogram Equalization) -- local contrast enhancement that, unlike a
    single global threshold, adapts to uneven lighting/glare across the
    image (very common on curved, glossy product packaging photographed
    at an angle). Verified directly against real curved-cup product
    photos during development: this variant alone took OCR output from
    completely unusable to recovering most label fields on photos where
    the other variants found nothing at all. Returns None if OpenCV
    isn't installed, so this variant is simply skipped rather than
...
def _generate_preprocess_variants(image: Image.Image) -> list:
    ...
    variants = [contrast_sharpened, binarized, gray]

    clahe_variant = _clahe_enhanced(gray)
...
# Tesseract page segmentation modes tried per image variant. The default
# (None -> tesseract picks automatically) works well on clean scans, but
# real photos with a text block surrounded by other packaging graphics
# (icons, logos, glossy highlights) often do noticeably better with PSM 6
# ("assume a single uniform block of text"), verified directly against
# real product photos during development -- the default mode sometimes
# found almost nothing on a photo where PSM 6 recovered most of the
# label. Trying both and keeping whichever parses best costs some extra
# time but meaningfully improves reliability on exactly the messy,
# angled, curved-surface photos this app needs to handle.
_TESSERACT_PSM_MODES = [None, "6"]


def _run_tesseract(image: Image.Image, psm: Optional[str] = None) -> str:
    config = f"--psm {psm}" if psm else ""
    return pytesseract.image_to_string(image, lang=_TESSERACT_LANGS, config=config)
...
def _ocr_single_variant(image: Image.Image, psm: Optional[str] = None) -> str:
    ...
    if _HAS_TESSERACT:
        try:
            text = _run_tesseract(image, psm=psm)
...
def extract_text(image: Image.Image) -> str:
    """Run OCR across multiple preprocessing variants (and, for
    Tesseract, multiple page-segmentation modes) of the image and return
    the text that yields the best parse. This is what makes real-world
    camera photos (uneven lighting, slight blur, glare, curved surfaces)
    meaningfully more reliable than a single fixed preprocessing pass.
    Returns '' if OCR isn't available or every attempt fails, so the
    ...
    psm_modes = _TESSERACT_PSM_MODES if _HAS_TESSERACT else [None]
    for variant in variants:
        for psm in psm_modes:
            text = _ocr_single_variant(variant, psm=psm)
            score = _score_text(text)
            if score > best_score:
                best_score = score
                best_text = text
            # EasyOCR (if that's what actually ran, e.g. tesseract
            # unavailable) ignores psm entirely and would just repeat the
            # identical call -- no point trying it more than once per
            # variant.
            if not _HAS_TESSERACT:
                break
...
_FIELD_PATTERNS = {
    ...
    "calories": [
        ...
        # Japanese: "エネルギー 180kcal" -- the more common wording
        rf"{_jp('エネルギー')}[^0-9]{{0,10}}{_NUMBER}",
        # Japanese: "熱量 354kcal" -- an equally valid, common alternate
        # wording for "energy" seen on real product labels (e.g. some
        # instant-noodle brands use this instead of エネルギー)
        rf"{_jp('熱量')}[^0-9]{{0,10}}{_NUMBER}",
    ],
    "total_carbs_g": [
    ...
    "protein_g": [
        ...
        # Japanese: "たんぱく質 5g" (also accept the 蛋白質 and たん白質
        # kanji variants -- all three spellings appear on real product
        # labels depending on manufacturer/era)
        rf"{_jp('たんぱく質')}[^0-9]{{0,10}}{_NUMBER}\s*g",
        rf"{_jp('蛋白質')}[^0-9]{{0,10}}{_NUMBER}\s*g",
        rf"{_jp('たん白質')}[^0-9]{{0,10}}{_NUMBER}\s*g",
    ],
    "total_fat_g": [
    ...
    "vitamin_b1_mg": [
        rf"vitamin\s*b1[^0-9]{{0,10}}{_NUMBER}\s*mg",
        # Japanese: "ビタミンB1 1.45mg"
        rf"{_jp('ビタミンb1')}[^0-9]{{0,10}}{_NUMBER}\s*mg",
    ],
    "vitamin_b2_mg": [
        rf"vitamin\s*b2[^0-9]{{0,10}}{_NUMBER}\s*mg",
        # Japanese: "ビタミンB2 0.25mg"
        rf"{_jp('ビタミンb2')}[^0-9]{{0,10}}{_NUMBER}\s*mg",
    ],
    "calcium_mg": [
...
# Some labels report sodium directly as ナトリウム but in *grams* rather
# than mg (the main sodium_mg pattern in _FIELD_PATTERNS only matches an
# "mg" unit) -- confirmed on a real product label. This fallback catches
# that case and converts g -> mg. The strict "number immediately followed
# by only whitespace then the unit letter" matching (no "m" allowed in
# between) means this naturally never collides with an "mg" value.
_SODIUM_GRAMS_PATTERN = rf"{_jp('ナトリウム')}[^0-9]{{0,10}}{_NUMBER}\s*g"
...
def parse_nutrition_text(text: str) -> ParsedNutrition:
    ...
    # Fallback: some labels report sodium directly as ナトリウム but in
    # grams rather than mg (the main pattern only matches an "mg" unit).
    # Checked before the salt-equivalent fallback below since a directly
    # stated sodium value is more precise than deriving one from salt.
    if result.sodium_mg is None:
        match = re.search(_SODIUM_GRAMS_PATTERN, normalized, flags=re.IGNORECASE | re.MULTILINE)
        if match:
            try:
                sodium_g = _clean_number(match.group(1))
                result.sodium_mg = round(sodium_g * 1000, 1)
            except (ValueError, IndexError):
                pass
