...
def _apply_parsed_to_session(parsed: ParsedNutrition) -> None:
    ...
    extended_mapping = {
        ...
        "Vitamin B1 (mg)": parsed.vitamin_b1_mg,
        "Vitamin B2 (mg)": parsed.vitamin_b2_mg,
...
if captured_image is not None:
    ...
    if image_obj is not None:
        # Only actually run OCR when this is a genuinely NEW image, not on
        # every script rerun. st.camera_input / st.file_uploader keep
        # returning the same captured file on every rerun until it's
        # explicitly replaced or cleared -- so without this guard, this
        # whole block (including _apply_parsed_to_session, which
        # overwrites the nutrition tables) would re-run every time the
        # person interacted with *anything* on the page, silently
        # wiping out manual edits to the tables below on every keystroke.
        image_hash = hashlib.md5(captured_image.getvalue()).hexdigest()
        already_processed = st.session_state.get("last_processed_image_hash") == image_hash
        ...
        if not already_processed:
            ...
            with st.spinner(spinner_label):
                ...
                # Mark this image as processed regardless of outcome, so a
                # failed/empty OCR result doesn't retry itself into a loop
                # on every rerun either -- the person can still switch
                # engines or re-upload to try again.
                st.session_state.last_processed_image_hash = image_hash

                if ocr_error:
                    st.session_state.last_ocr_message = ("error", ocr_error)
                else:
                    st.session_state.raw_ocr_text = raw_text
                    st.session_state.ocr_fields_found = parsed.fields_found()
                    st.session_state.current_source = "Manual/OCR"
                    if parsed.is_empty():
                        st.session_state.last_ocr_message = (
                            "warning",
        ...
        # Always (re-)display the last OCR outcome for this image, even on
        # reruns where OCR itself didn't re-run -- so the feedback doesn't
        # just vanish the moment the person edits something else.
        if st.session_state.get("last_ocr_message"):
            level, message = st.session_state.last_ocr_message
...
with reset_col:
    if st.button("↺ Reset values"):
        ...
        st.session_state.extracted_fields = set()
        st.session_state.extracted_fields_extended = set()
        st.session_state.last_ocr_message = None
        st.session_state.nutrition_table_version += 1
        st.rerun()
        ...
        try:
            new_id = save_food(
                FoodEntry(
                    ...
                    vitamin_b1_mg=ev["Vitamin B1 (mg)"],
                    vitamin_b2_mg=ev["Vitamin B2 (mg)"],
...
facts = NutritionFacts(
    ...
    vitamin_b1_mg=extended_vals["Vitamin B1 (mg)"],
    vitamin_b2_mg=extended_vals["Vitamin B2 (mg)"],
...
with st.expander("📋 Full nutrition panel (scaled to servings entered above)"):
    panel_rows = [
        ...
        ("Vitamin B1", f"{scaled_facts.vitamin_b1_mg:.2f} mg"),
        ("Vitamin B2", f"{scaled_facts.vitamin_b2_mg:.2f} mg"),
